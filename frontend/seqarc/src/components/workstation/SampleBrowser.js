...
const SampleBrowser = (props) => {
  ...
  useEffect(() => {
    Axios.get("http://127.0.0.1:80/api/category/findall").subscribe(
      (response) => {
    ...
    <div className="sample-browser-container">
            ...
            {samples.map((sample, index) => {
              return (
                <tr
                  key={index}
                  onClick={() =>
                    handleClickOnSample(sample.checksum, sample.name)
                  }
                >
                  <td>{sample.name}</td>
      ...
      <div className="browser-footer">
        <div className="browser-footer-buttons">
          {props.token ? (
            <div className="browser-footer-button" onClick={uploadModalToggle}>
              Upload
            </div>
          ) : (
            <div />
        ...
        <div className="audition-vol">
          ...
          <div className="vol-ticks">
            <div
              className={`vol-tick vol-tick-0 vol-tick-${
                volInt >= 0 ? "on" : "off"
              }`}
            />
            <div
              className={`vol-tick vol-tick-1 vol-tick-${
                volInt >= 1 ? "on" : "off"
              }`}
            />
            <div
              className={`vol-tick vol-tick-2 vol-tick-${
                volInt >= 2 ? "on" : "off"
              }`}
            />
            <div
              className={`vol-tick vol-tick-3 vol-tick-${
                volInt >= 3 ? "on" : "off"
              }`}
            />
            <div
              className={`vol-tick vol-tick-4 vol-tick-${
                volInt >= 4 ? "on" : "off"
              }`}
            />
          </div>
